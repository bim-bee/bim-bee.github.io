(function() {
  ...
  function deriveArticleSlug() {
    const pathParts = (window.location.pathname || '').split('/');
    const fileName = pathParts[pathParts.length - 1] || 'post.html';
  ...
  const articleSlug = deriveArticleSlug();
  ...
  if (recipientId) {
    ...
    config.campaign_name = `bimblog-${articleSlug}`;
    config.campaign_content = articleSlug;
  }
